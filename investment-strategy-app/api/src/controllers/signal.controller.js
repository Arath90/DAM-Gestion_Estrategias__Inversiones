import boom from '@hapi/boom';
import mongoose from 'mongoose';
import Signal from '../models/signal.js';

export const getSignals = async (req, res, next) => {
  try {
    const items = await Signal.find().sort({ ts: -1 });
    res.status(200).json(items);
  } catch (err) { next(err); }
};

export const getSignalById = async (req, res, next) => {
  try {
    const { id } = req.params;
    if (!mongoose.Types.ObjectId.isValid(id))
      throw boom.badRequest('El parámetro id no es un ObjectId válido.');
    const item = await Signal.findById(id);
    if (!item) throw boom.notFound('Signal no encontrada.');
    res.status(200).json(item);
  } catch (err) { next(err); }
};

export const createSignal = async (req, res, next) => {
  try {
    const { instrument_id } = req.body;
    if (!mongoose.Types.ObjectId.isValid(instrument_id))
      throw boom.badRequest('instrument_id no es un ObjectId válido.');
    const created = await Signal.create(req.body);
    res.status(201).json(created);
  } catch (err) {
    if (err.code === 11000) return next(boom.conflict('Señal duplicada para (strategy_code, instrument_id, ts, action).'));
    next(err);
  }
};

export const updateSignal = async (req, res, next) => {
  try {
    const { id } = req.params;
    if (!mongoose.Types.ObjectId.isValid(id))
      throw boom.badRequest('El parámetro id no es un ObjectId válido.');
    if (req.body.instrument_id && !mongoose.Types.ObjectId.isValid(req.body.instrument_id))
      throw boom.badRequest('instrument_id no es un ObjectId válido.');
    const updated = await Signal.findByIdAndUpdate(id, req.body, {
      new: true, runValidators: true
    });
    if (!updated) throw boom.notFound('Signal no encontrada.');
    res.status(200).json(updated);
  } catch (err) {
    if (err.code === 11000) return next(boom.conflict('Señal duplicada para (strategy_code, instrument_id, ts, action).'));
    next(err);
  }
};

export const deleteSignal = async (req, res, next) => {
  try {
    const { id } = req.params;
    if (!mongoose.Types.ObjectId.isValid(id))
      throw boom.badRequest('El parámetro id no es un ObjectId válido.');
    const deleted = await Signal.findByIdAndDelete(id);
    if (!deleted) throw boom.notFound('Signal no encontrada.');
    res.status(204).send();
  } catch (err) { next(err); }
};
