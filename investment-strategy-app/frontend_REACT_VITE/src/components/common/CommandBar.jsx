...
import { DailyPnlsAPI, RiskLimitsAPI, OrdersAPI, CandlesAPI, InstrumentsAPI, SignalsAPI } from '../../services/odata';
...
export default function CommandBar({ open, onClose, onGoto }) {
  ...
  // Buscar en entidades clave con OData $filter contains(...)
  useEffect(() => {
    let alive = true;
    const run = async () => {
      ...
      try {
        ...
        setResults([
          ...
          ...sig.map(x => ({ type: 'Señal', id: x.ID, label: `${x.strategy_code} • ${x.action}`, payload: x })),
        ]);
      } catch { /* silencioso */ } finally { if (alive) setLoading(false); }
    };
    ...
    <div className="fixed inset-0 z-50">
      ...
      <div className="mx-auto mt-24 max-w-2xl bg-[#0f172a] text-white rounded-xl shadow-2xl p-3">
        <input
          ...
          placeholder="Buscar… (instrumento, orden, señal)  •  ↑/↓ para navegar, Enter para abrir"
        ...
        <div className="max-h-96 overflow-y-auto divide-y divide-white/5">
          ...
          {q && (loading ? <div className="p-4 opacity-80">Buscando…</div> :
